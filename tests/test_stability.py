...
class TestNoRegressionReward:
    ...
    def test_rr_a1_correct(self) -> None:
        ...
        r = compute_no_regression_reward("A", "A", "mcq", a1_is_correct=True, format_valid=True)
        assert r == 1.0

    def test_rw_a1_correct(self) -> None:
        """A1 correct, A2 wrong → RW → -3.0."""
        r = compute_no_regression_reward("B", "A", "mcq", a1_is_correct=True, format_valid=True)
        assert r == -3.0

    def test_wr_a1_wrong(self) -> None:
        """A1 wrong, A2 correct → WR → +2.0."""
        r = compute_no_regression_reward("A", "A", "mcq", a1_is_correct=False, format_valid=True)
        assert r == 2.0

    def test_ww_a1_wrong(self) -> None:
        """A1 wrong, A2 wrong → WW → 0.0."""
        r = compute_no_regression_reward("B", "A", "mcq", a1_is_correct=False, format_valid=True)
        assert r == 0.0

    def test_format_invalid(self) -> None:
        r = compute_no_regression_reward("A", "A", "mcq", a1_is_correct=True, format_valid=False)
        assert r == 0.0
    ...
    def test_empty_mcq_is_rw(self) -> None:
        ...
        r = compute_no_regression_reward("", "A", "mcq", a1_is_correct=True, format_valid=True)
        assert r == -3.0

    def test_mcq_parens_same_as_plain(self) -> None:
        """(A) and A should both extract to 'A' → identical edit distance."""
        r = compute_minimal_edit_reward("(A)", "A", "A", "mcq", format_valid=True)
        assert r == 1.0
...
class TestMinimalEditReward:
    ...
    def test_a1_wrong_returns_zero(self) -> None:
        ...
        r = compute_minimal_edit_reward("B", "A", "A", "mcq", format_valid=True)
        assert r == 0.0

    def test_a2_wrong_returns_zero(self) -> None:
        """A2 wrong → 0.0."""
        r = compute_minimal_edit_reward("A", "B", "A", "mcq", format_valid=True)
        assert r == 0.0

    def test_format_invalid(self) -> None:
        r = compute_minimal_edit_reward("A", "A", "A", "mcq", format_valid=False)
        assert r == 0.0

    def test_lambda_edit_controls_penalty(self) -> None:
        """Higher lambda_edit means more penalty for edits."""
        r_low = compute_minimal_edit_reward(
            "cat", "cat", "cat", "open", format_valid=True, lambda_edit=0.1
        )
        r_high = compute_minimal_edit_reward(
            "cat", "cat", "cat", "open", format_valid=True, lambda_edit=0.9
        )
    ...
    def test_reward_clamped_at_zero(self) -> None:
        ...
        r = compute_minimal_edit_reward(
            "Yes", "Yes", "Yes", "yesno", format_valid=True, lambda_edit=2.0
        )
        assert r >= 0.0
