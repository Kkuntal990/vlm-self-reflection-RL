...
class SelfReflectionGRPOTrainer:
    ...
    def __init__(
        ...
        # Two-adapter mode: cache the flag (read often) and the trainable
        # adapter name. In default (single-adapter) mode this is "default";
        # in two-adapter mode it's "f1_a2_expert" (the trainable sibling of
        # the frozen "a1_expert"). The trainer:
        #   - skips the A1 forward + A1 KL + A1 policy loss
        #   - routes generation per-turn via ``adapter_callback`` (HF
        #     ``set_adapter`` and a vLLM weight resync)
        #   - uses ``self._train_adapter`` everywhere it currently uses
        #     "default" so save_pretrained / set_adapter calls target the
        #     trainable one
        self._two_adapter_mode = bool(getattr(config, "two_adapter_mode", False))
        self._train_adapter = "f1_a2_expert" if self._two_adapter_mode else "default"
        if self._two_adapter_mode:
            logger.info(
                "Two-adapter mode ENABLED: a1_expert FROZEN, "
                "f1_a2_expert TRAINABLE. A1 policy loss + A1 KL are dropped."
            )
            ...
            if bool(getattr(config.rollout, "single_turn_a1", False)):
                raise ValueError(
                    "two_adapter_mode is incompatible with --single_turn_a1. "
                    "Single-turn mode trains A1 only; two-adapter mode freezes A1. "
                    "Together they train nothing."
                )
    ...
    def train_step(self, batch: list[dict]) -> SelfReflectionTrainStepResult:
        ...
        #
        # Two-adapter mode skips the pre-rollout sync — the per-turn
        # ``adapter_callback`` handles the wake/sync/sleep cycle for A1
        # (a1_expert merged) and F1+A2 (f1_a2_expert merged) separately.
        if self.vllm_engine is not None and not self._two_adapter_mode:
        ...
        adapter_callback = (
            self._build_adapter_callback(gen_model) if self._two_adapter_mode else None
        )
        ...
        # Two-adapter mode: restore the trainable adapter as active so the
        # downstream training forward / KL / save paths see the right
        # weights. The callback may have left A1's adapter active after
        # the last rollout turn (if A2 wasn't routed back).
        if self._two_adapter_mode:
            self._set_active_adapter(gen_model, self._train_adapter)
        ...
        single_turn_a1 = bool(getattr(self.config.rollout, "single_turn_a1", False))
        # Two-adapter mode: A1 is generated by a frozen sibling adapter
        # (a1_expert), gets no policy-loss / KL contribution, and therefore
        # does not need a forward pass at all. Saves ~1/3 of the inner-loop
        # compute and removes the A1-shape advantage from the gradient.
        skip_a1_forward = self._two_adapter_mode
        if skip_a1_forward:
            a1_pretok = None
        else:
        ...
        with torch.no_grad():
            ...
            for mb_s in range(0, n_traj, inner_mini_bs):
                ...
                elif skip_a1_forward:
                    a2_lps, fb_lps = self._forward_from_pretokenized_multi(
                        [a2_pretok, f1_pretok], unwrapped_model, mb_s, mb_e
                    )
            ...
            # The adapter state MUST be restored to "default" (or
            # ``self._train_adapter`` in two-adapter mode) / enabled before
            # leaving this block — downstream code (training forward, vLLM
            # sync, checkpoint save) assumes the trainable adapter is active.
                ...
                try:
                    for mb_s in range(0, n_traj, inner_mini_bs):
                        ...
                        if single_turn_a1:
                            ...
                            a2_lps, fb_lps = self._forward_from_pretokenized_multi(
                                [a2_pretok, f1_pretok], ref_m, mb_s, mb_e
                            )
            ...
            if single_turn_a1:
                ...
                # Two-adapter mode: A1 is frozen on a1_expert (no policy
                # loss, no KL). Response head trains on A2 only — old_resp
                # is just old_a2.
                old_a1_lps = None
                old_a2_lps = old_a2_lps_list
                old_resp_lps = old_a2_lps_list
        ...
        for inner_epoch in range(num_inner):
            ...
            for mb_start in range(0, n_traj_inner, inner_mini_bs):
                ...
                if single_turn_a1:
                    ...
                    # Two-adapter mode: A1 is frozen, no forward pass needed.
                    mb_a2_lps, mb_fb_lps = self._forward_from_pretokenized_multi(
                        [a2_pretok, f1_pretok],
                        inner_model,
                        mb_start,
                        mb_end,
                ...
                for j in range(mb_end - mb_start):
                    ...
                    if single_turn_a1:
                        ...
                        # Two-adapter mode: response loss runs on A2 only.
                        # A1 was generated by the FROZEN a1_expert sibling
                        # adapter; gradient on the trainable f1_a2_expert
                        # cannot affect A1 generation, so applying a policy
                        # loss to A1 log-probs would just be a meaningless
                        # KL pull on the trainable adapter (KL is handled
                        # explicitly below; we don't conflate it with the
                        # response head).
                        a2_new = mb_a2_lps[j]
                        a2_ratio_raw = torch.nan_to_num(
                            a2_new - old_a2_lps[ti].detach(),
                            nan=0.0,
                            posinf=20.0,
                            neginf=-20.0,
                        )
                        ...
                        traj_resp_loss = (
                            -torch.min(
                                a2_ratio * resp_advantages[ti],
                                a2_clipped * resp_advantages[ti],
                            ).sum()
                            / a2_max_len
                        )
                        self._resp_total_tokens += a2_ratio.numel()
                        self._resp_clipped_tokens += (a2_ratio != a2_clipped).sum().item()
                    ...
                    # Two-adapter mode: A1 KL is dropped — the trainable
                    # f1_a2_expert adapter never participates in A1 generation,
                    # so applying KL on A1 log-probs would be a no-op on the
                    # quantity it's meant to anchor.
                    if (
                        self.config.kl_coeff > 0
                        and ref_a2_lps is not None
                        and ref_fb_lps is not None
                    ):
                        ...
                        traj_kl_loss = a2_kl_c * a2_kl + fb_kl_c * f1_kl
                        if not skip_a1_forward and ref_a1_lps is not None:
                            a1_kl_max_len = float(
                                self.config.rollout.a1_max_completion_length or 200
                            )
                            ...
                            traj_kl_loss = traj_kl_loss + a1_kl_c * a1_kl
                    else:
    ...
    def _set_active_adapter(self, peft_model: Any, name: str) -> None:
        """Set the active adapter on an unwrapped PEFT model, no-op if already active.

        Used by the two-adapter rollout callback to switch between
        ``a1_expert`` (frozen) and ``f1_a2_expert`` (trainable) without
        triggering the PEFT machinery on every redundant call.
    ...
    def _build_adapter_callback(self, gen_model: Any) -> Any:
        """Construct the per-turn adapter routing callback for two-adapter rollout.

        The returned callable is fired once before each generation turn
        (with one of "a1", "f1", or "a2"). It:

          1. Picks the target adapter ("a1_expert" for A1, otherwise the
             trainable "f1_a2_expert").
          2. No-ops if the target is already active (avoids redundant
             merge → load_weights cycles between F1 and A2).
          3. On adapter change, switches the HF model's active adapter,
             puts vLLM to sleep, wakes it for weights, re-syncs the new
             adapter's merged weights, then wakes vLLM for generation.

        The dual-merge approach (sleep → sync → wake) keeps vLLM's base
        weights in lock-step with whichever adapter is active. The cost
        is one extra wake/sleep per rollout step (when adapter changes
        from f1_a2_expert back to a1_expert at the start of each batch);
        F1→A2 is free because the same adapter stays active.

        Args:
            gen_model: Unwrapped PEFT model used for generation.

        Returns:
            Callable[[str], None] usable as ``adapter_callback`` in
            ``generate_self_reflection_rollout``.
        """

        def callback(turn: str) -> None:
            target = "a1_expert" if turn == "a1" else self._train_adapter
    ...
    def _save_checkpoint(self, path: Path) -> None:
        ...
        if self._two_adapter_mode:
            unwrapped.set_adapter(self._train_adapter)
            unwrapped.save_pretrained(path, selected_adapters=[self._train_adapter])
        elif self._use_ref_adapter:
