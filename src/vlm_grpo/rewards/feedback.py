...
def compute_downstream_aware_reward(
    ...
    **Improvement mode** (use_improvement_reward=True):
        ...
        dominate the advantage, directly rewarding corrective feedback
        and penalizing regressive feedback. Fixes the inverted advantage
        problem where WR trajectories had negative mean advantage under
        transition-shaped rewards.
    ...
    if use_improvement_reward:
        # R_improve = correctness(A2) - correctness(A1)
        # Each is +1 (correct) or -1 (wrong), so delta ∈ {-2, 0, +2}
